...
## Updates the player dmg label in the HUD
## Called by: Game.gd in _process(_delta)
func set_player_dmg(dmg: float) -> void:
	if dmg == null: 
		return
	player_dmg_label.text = "Dmg: %.2f" % dmg

## Updates the player movement speed label in the HUD
## Called by: Game.gd in _process(_delta)
...
## Updates the time in level label in the HUD
## Called by: Game.gd in _process(_delta)
...
## Updates the label to show the amount of difficulty points on screen from enemies spawned in on the HUD
## Called by: Game.gd in _process(_delta)
...
## Updates the level number in the HUD
## Called by: Game.gd in _process(_delta)
